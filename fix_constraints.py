...
def fix_constraints(model,likelihood,kernel_type,num_task):
    ...
    if kernel_type == "rbf":
        likelihood.register_constraint("raw_noise", gpytorch.constraints.Interval(1e-7,5e-1))                                         #(1e-4,1)
        likelihood.register_constraint("raw_task_noises", gpytorch.constraints.Interval(1e-3,1))                                   #(1e-1,2)
       
        model.covar_module.kernels[0].base_kernel.register_constraint("raw_lengthscale", gpytorch.constraints.Interval(10,100))    #(1.5,50)
        model.covar_module.kernels[0].register_constraint("raw_outputscale", gpytorch.constraints.Interval(10,50))                #(1e-3,1)
        model.covar_module.kernels[1].register_constraint("raw_bias", gpytorch.constraints.Interval(1e-9,1e-3))                       #(1e-6,1)
