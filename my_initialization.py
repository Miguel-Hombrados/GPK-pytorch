...
def my_initialization(model,kernel_type,task_num):
  ...
  #  }
         ...
         model.likelihood.task_noises = 1e-8*torch.ones(task_num)
    ...
    # }
        model.likelihood.noise = torch.tensor(0.01)
        model.likelihood.task_noises = 1e-8*torch.ones(task_num)
        model.covar_module.kernels[0].base_kernel.outputscale = 1e-5*torch.ones(task_num)
        model.covar_module.kernels[0].base_kernel.lengthscale = 1*torch.ones(task_num,1,1)
        model.covar_module.kernels[1].bias = 1e-7*torch.ones(task_num,1,1)
